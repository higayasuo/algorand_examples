...
from algosdk.future.transaction import (
    ApplicationNoOpTxn, PaymentTxn, calculate_group_id)
...
from helper import (create_algod_client,
                    ...
                    read_global_state, send_wait_transaction, sign_send_wait_transaction)
...
def init_escrow_asc1(client: AlgodClient, private_key: str, app_id: int, asset_id: int):
    ...
    txn = ApplicationNoOpTxn(sender, params, app_id,
                             app_args, foreign_assets=[asset_id])
...
def buy(client: AlgodClient, private_key: str, app_id: int, asset_id: int,
        owner_address: str):
    app_args = [escrow_asc1.AppMethods.buy]

    helper.call_app(client, private_key, app_id, app_args=app_args,
                    foreign_assets=[asset_id], accounts=[owner_address])
    print(read_global_state(client, app_id))

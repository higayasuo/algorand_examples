...
from escrow.escrow07_asc1 import (
    approval_program,
    clear_state_program,
    global_schema,
    local_schema,
...
@pytest.fixture
def asset_id2(client: AlgodClient, app_address: str) -> Iterator[int]:
    id = helper.create_asset(
        client,
        test1_private_key,
        asset_name="ASA",
...
def test_buy_opt_in_asset_id(
    ...
    with pytest.raises(AlgodHTTPError) as e:
        ...
        txn1 = AssetOptInTxn(test2_address, sp, index=asset_id2)
        txn2 = ApplicationNoOpTxn(
            test2_address,
            sp,
            app_id,
            app_args=[AppMethods.transfer_asset],
            foreign_assets=[asset_id2],
            accounts=[test1_address],
        )
        txn3 = PaymentTxn(test2_address, sp, receiver=test1_address, amt=1000000)
        sign_send_wait_group_transactions(
